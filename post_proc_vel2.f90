...
subroutine calculate_integral(Pcon,j_start,j_end,k_start,k_end,i_start,i_end,dsd_pcon)

integer, intent(in) :: j_start,j_end,k_start,k_end,i_start,i_end
integer             :: ipcon,npcon=16
...
!Parameters
  INTEGER,PARAMETER                           :: nproc=72
  REAL(rprec),PARAMETER                       :: TINYS=1e-24
  INTEGER,PARAMETER                           :: ny=150
  INTEGER,PARAMETER                           :: nz_tot=361
  INTEGER,PARAMETER                           :: nz = (361-1)/nproc +1
  ...
  REAL(rprec),PARAMETER                      :: z_i=1.2_rprec
...
! close(30)
!  enddo
!  print *, "w= ", maxval(-avg_w*u_star)
!
505 format(18e15.7)
!deallocate(Pcon0,Pcon)

open(unit=20,FILE='av_vel.out',form='unformatted')
!open(unit=21,FILE='av_P1_15_3_4.dat')
read(20) avg_u(:,:,1:nz_tot),avg_v(:,:,1:nz_tot),avg_w(:,:,1:nz_tot)
close(20)
