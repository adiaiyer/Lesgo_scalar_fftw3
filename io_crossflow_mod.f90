...
use param,only:output,dt,c_count,S_FLAG,SCAL_init,jt,jan_diurnal_run,PCon_FLAG,PCon_init,Pcon_count,&
               checkpoint_data,checkpoint_nskip
...
if (S_FLAG .and. Pcon_FLAG) then
  write(fname,'(A,I7.7,A,I4.4,A)')path//'output_data/vel_pcon_',jt_total,'_',2000+coord,'.out'
endif
...
if(S_FLAG .and. PCON_FLAG) then
  write(2000+coord)u(:,:,1:nz),v(:,:,1:nz),w(:,:,1:nz),Pcon(:,:,1:nz,1:npcon),&
        breakage_freq(:,:,1:nz,1:npcon),Re(:,:,1:nz,1:npcon),&
        dissip(:,:,1:nz),Rhs_pbe(:,:,1:nz,1:npcon),Rhs_Pcon(:,:,1:nz,1:npcon),Cs_opt2
endif
close(2000+coord)
