...
integer,parameter:: nx=384,ny=150,nz=(361-1)/nproc + 1
!AA End of change
...
real(kind=rprec),parameter::L_x=2_rprec,L_y=0.78125_rprec
!real(kind=rprec),parameter::L_x=10._rprec,L_y=5._rprec
real(rprec),parameter::z_i=1.2_rprec, L_z=z_i/nproc
...
!!!!!!!! Changed by AA
integer,parameter::nsteps=20000 !simulation steps
!!!!!!! Ends Here
...
!!!!!!! Changed by AA
real(rprec),parameter::dt_dim=0.0002_rprec !dimensional time step in seconds
!!!!!!! Ends here
...
integer,parameter :: PCon_init=5000
...
real (rprec), parameter :: sigma=1.9e-2_rprec, dmin=20e-6_rprec, dmax=1000e-6_rprec, d_min=1e-6_rprec,&
                           rho_c=864._rprec,nu_c=1e-6_rprec,mu_ratio=10._rprec,k_b=0.3_rprec,nu_d=1e-5_rprec
!real (rprec), parameter ::d_diameter=(dmax-dmin)/(npcon-2._rprec)
real (rprec), parameter :: ratio = 1._rprec/14._rprec
real (rprec) , parameter :: d_diameter=(50._rprec)**ratio
REAL(KIND=RPREC),parameter :: C1=2500
...
real(kind=rprec),parameter,dimension(npcon) :: settling_vel = (/3.34780488313695e-05_rprec/u_star, 5.85024736402885e-05_rprec/u_star,&
                                                               0.000102178391780517_rprec/u_star, 0.000178294781132529_rprec/u_star, &
                                                               0.000310602687074315_rprec/u_star, 0.000539545177876969_rprec/u_star,&
                                                               0.000932633504185896_rprec/u_star, 0.00159883248975775_rprec/u_star,&
                                                               0.00270457988866154_rprec/u_star, 0.00448327639492178_rprec/u_star,&
                                                               0.00722477125790607_rprec/u_star, 0.0112387659082189_rprec/u_star,&
                                                               0.0168125499629259_rprec/u_star,  0.0242007627271716_rprec/u_star,&
                                                               0.0336604281398371_rprec/u_star,0._rprec/u_star/)
...
real(kind=rprec),parameter,dimension(npcon) :: Q_src=(/(0._rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
(0._rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)) ,(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)), &
(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(0._rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)), &
(0._rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(0._rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(6.051e4_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
(0_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star))/)
...
!   (6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star))/)!,(6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)), &
!(6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
!(6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
!(6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(6.45e-9_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),&
...
!real(kind=rprec),parameter,dimension(npcon):: Q_src=(/(0._rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star))/)
!    (72.93e-3_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(1.0e-4_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)), &
!    (9.0e-5_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star)),(25.2e-3_rprec/(dx*z_i*dy*z_i*dz*z_i))/(PCon_scale/(z_i/u_star))/)

!AA Change ends here
! timesteps to begin and end point source
integer,parameter :: ini_src=5000
...
!AA Change density ratio
!real (kind=rprec), parameter,dimension(npcon) :: densratio_pcon=(/1.4_rprec/1000._rprec,1000._rprec/1000._rprec/)!,1000._rprec/1000._rprec,1000._rprec/1000._rprec,1000._rprec/1000._rprec,&
!1000._rprec/1000._rprec,1000._rprec/1000._rprec,1000._rprec/1000._rprec,1000._rprec/1000._rprec,1000._rprec/1000._rprec/)
!real (kind=rprec), parameter,dimension(npcon) :: densratio_pcon=(/99.2_rprec/1027.7_rprec,858._rprec/1027.7_rprec,1027.7_rprec/1027.7_rprec,927.1_rprec/1027.7_rprec,891._rprec/1027.7_rprec/)

real(kind=rprec),parameter,dimension(npcon) :: densratio_pcon=(/rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,&
                                                rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,&
                                       rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,rho_c/1030._rprec,1.4_rprec/1030._rprec/)
!AA Change ends here
...
!real (kind=rprec), parameter,dimension(npcon) :: V_pcon=(/1._rprec/1.4_rprec,1._rprec/1000._rprec/)!,1._rprec/1000._rprec,1._rprec/1000._rprec,1._rprec/1000._rprec,&
!1._rprec/1000._rprec,1._rprec/1000._rprec,1._rprec/1000._rprec,1._rprec/1000._rprec,1._rprec/1000._rprec/)
!AA Change v_pcon
!real (kind=rprec), parameter,dimension(npcon) :: V_pcon=(/1.125e-11_rprec/)
!real (kind=rprec), parameter,dimension(npcon) :: V_pcon=(/1._rprec/99.2_rprec,1._rprec/858._rprec,1._rprec/1027.7_rprec,1._rprec/927.1_rprec,1._rprec/891._rprec/)
!real(kind=rprec),parameter,dimension(npcon) :: V_pcon=(/1._rprec/1.4_rprec,1._rprec/1000._rprec/)
!real(kind=rprec),parameter,dimension(npcon) :: V_pcon=(/1.125e-13_rprec/V_pcon0,7.0233196e-13_rprec/V_pcon0,2.08624828e-12_rprec/V_pcon0,&
 !                                                      4.629629629629e-12_rprec/V_pcon0,8.68535664e-12_rprec/V_pcon0,& 
 !                                                  1.46063100e-11_rprec/V_pcon0,2.27453703e-11_rprec/V_pcon0,3.34554183e-11_rprec/V_pcon0,&
 !                                                    4.70893347e-11_rprec/V_pcon0,6.4e-11_rprec/V_pcon0,1.125e-11_rprec/)

real(kind=rprec),parameter,dimension(npcon) :: V_pcon=(/4.1888e-15_rprec,9.6862e-15_rprec,2.2398e-14_rprec,5.1794e-14_rprec,&
1.1977e-13_rprec,2.7696e-13_rprec,6.4044e-13_rprec,1.481e-12_rprec,3.4246e-12_rprec,7.9191e-12_rprec,1.8312e-11_rprec,& 
               4.2345e-11_rprec,9.7919e-11_rprec,2.2643e-10_rprec,5.236e-10_rprec,4.1888e-15_rprec/)
...
logical,parameter :: FRINGE_FLAG = .TRUE.
