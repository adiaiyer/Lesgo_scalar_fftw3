...
use sgsmodule,only:Nu_t,magS,dissip
...
!real(kind=rprec),dimension(:, :, :), allocatable :: beta_scal,Pr_
...
!real(kind=rprec),dimension(:, :, :), allocatable :: dTdz,dqdz
...
!real(kind=rprec),dimension(: , :, :), allocatable :: RHS_Tf,RHS_T,RHS_qf,RHS_q
real(kind=rprec), dimension(ld,ny,$lbz:nz):: sgs_t3,sgs_q3 !defines the surface sgs flux
!real(kind=rprec), dimension(:, :, :), allocatable :: sgs_t3,sgs_q3 !defines the surface sgs flux
...
!real(kind=rprec),dimension(:, :), allocatable ::L,wstar 
...
!real(kind=rprec),dimension(:, :), allocatable::T_s_filtered
...
real(kind=rprec),dimension(ld,nx,$lbz:nz) :: Pcon_neg
...
!REAL(kind=rprec),DIMENSION(:, :, :), allocatable  :: Kc_t          ! 3D matrix of SGS diffusivity for pollen
!REAL(kind=rprec),DIMENSION(:, :, :), allocatable  :: dPCondz       ! Vertical derivatives (needed also for output)
!REAL(kind=rprec),DIMENSION(:, :, :, :), allocatable  :: RHS_PConf,RHS_PCon ! RHS for PCon equation
!REAL(kind=rprec),DIMENSION(:, :, :), allocatable  :: sgs_PCon3     ! Defines the sgs vertical flux
!REAL(kind=rprec),DIMENSION(:, :, :), allocatable  :: res_PCon3     ! Defines the resolved vertical flux
!REAL(kind=rprec),DIMENSION(:, :, :), allocatable  :: sgs_PCon1     ! Defines the sgs x-direction flux
!REAL(kind=rprec),DIMENSION(:, :, :), allocatable  :: Cs2Sc         ! Dynamic coefficient for scalar equation

!REAL(kind=rprec),DIMENSION(:, :), allocatable          :: P_surf_flux   ! Surface pollen flux     
!REAL(kind=rprec),DIMENSION(:, :), allocatable          :: deposition    ! Surface pollen deposition (this is actually the net flux=deposition-source)
!REAL(kind=rprec),DIMENSION(:, :), allocatable          :: P_surf_flux_dep! Surface pollen flux for Cr=0 everywhere
!REAL(kind=rprec),DIMENSION(:, :), allocatable          :: Real_dep      ! This is the real deposition (using Cr=0 everywhere)
!REAL(kind=rprec)                           :: flux_out,flux_out_prev

! Variables for interpolation of velocity field
!REAL(kind=rprec),DIMENSION(:, :), allocatable          :: matrix_x      ! Matrix for x 4direction
!REAL(kind=rprec),DIMENSION(:, :), allocatable          :: matrix_y      ! Matrix for y direction
!REAL(kind=rprec),DIMENSION(:), allocatable             :: dvector_x     ! Derivative vector for x direction
!REAL(kind=rprec),DIMENSION(:), allocatable             :: dvector_y     ! Derivative vector for y direction
...
!++++++++++++++++++++++++++++++++++
!DY Variables added by Di Yang
!REAL(kind=rprec),dimension(:, :, :), allocatable :: beta_pcon
!++++++++++++++++++++++++++++++++++

!++++++++++++++++++++++++++++++++++
!AA Add variables to solve PBE
!real(kind=rprec),dimension(npcon) :: diameter
!integer                           :: ipcon
!diameter (1)=dmin
!do ipcon=2,npcon
!  diameter(ipcon) = dmin + (ipcon-1)*dx
!enddo
real(kind=rprec), dimension(ld,ny,$lbz:nz,npcon) ::Re,breakage_freq,Rhs_pbe,fitl
...
!DY            +(w_m(i,j,k)*dsdz_m(i,j,k)+w_m(i,j,k+1)*dsdz_m(i,j,k+1))/2._rprec
         if(OCEAN_FLAG .and. STOKES_FLAG) then
            RHS_m(i,j,k)=(u_m(i,j,k)+ust(k)+ucross)*dsdx_m(i,j,k)+v_m(i,j,k)*dsdy_m(i,j,k)&
...
!DY!        RHS_m(i,j,Nz)=u_m(i,j,Nz)*dsdx_m(i,j,Nz)+v_m(i,j,Nz)*dsdy_m(i,j,Nz)
         if(OCEAN_FLAG .and. STOKES_FLAG) then
            RHS_m(i,j,1)=(u_m(i,j,1)+ust(1)+ucross)*dsdx_m(i,j,1)+v_m(i,j,1)*dsdy_m(i,j,1)&
...
!DY        RHS_m(i,j,Nz)=u_m(i,j,Nz)*dsdx_m(i,j,Nz)+v_m(i,j,Nz)*dsdy_m(i,j,Nz)
         if(OCEAN_FLAG .and. STOKES_FLAG) then
            RHS_m(i,j,Nz)=(u_m(i,j,Nz)+ust(Nz)+ucross)*dsdx_m(i,j,Nz)+v_m(i,j,Nz)*dsdy_m(i,j,Nz)
         else
...
SUBROUTINE pollen_all_in_one2
  ...
  INTEGER:: i, j, ipcon
  INTEGER :: jz,ios,counter,jx,jy,jt_new
...
!  REAL(kind=rprec), DIMENSION(ld,ny,$lbz:nz) :: Pcon_neg
$if ($MPI)
  $define $lbz 0
$else
  $define $lbz 1
...
!    CALL pollen_RHS_calc2(PCon,dPCondz,PCon_s,zo_PCon,RHS_PCon,sgs_PCon3,res_PCon3,sfc_flux)
    CALL pollen_RHS_calc2(PCon,dPCondz,PCon_sfc_temp,zo_PCon,RHS_PCon,sgs_PCon3,res_PCon3,sfc_flux,sgs_PCon1,Rhs_pbe)  
!AA 
! do ipcon=1,npcon-1
!   do jz=1,nz-1
!      do jy=1,ny
!        do jx=1,nx
!          PCon(jx,jy,jz,ipcon) = PCon(jx,jy,jz,ipcon) + Rhs_pbe(jx,jy,jz,ipcon)
!        enddo
!      enddo
!    enddo
!  enddo
!AA Ends
    ...
    IF (((jt_total .eq. PCon_init) .or. (pointsource .and. (jt_total .eq. ini_src))) &
...
!AA BEgin here to check if pcon becomes negative
  !  WHERE ( Pcon(:,:,:,npcon-1) .lt. 0._rprec)
  !     Pcon_neg = Pcon(:,:,:,npcon-1) 
  !  END WHERE
...
subroutine pollen_RHS_calc2(scalar,dsdz,S_Surf,z_os,RHS,sgs_vert,res_vert,surf_flux_current,sgs_x,Rhs_pbe_in)
...
!AA
  ...
  integer:: i, j, k, jz, ipcon, jpcon !AA Added variable jpcon
  integer:: jz_min,ubc_jz
  real(kind=rprec):: surf_flux_current
!AA CHange dimension(nx,ny). Added term1 also as 2 D
!  real(kind=rprec):: term_alpha,term_xi,term_tao,term_y,term_r,term_omega
  real(kind=rprec), dimension(nx,ny) :: term1, &
     term_alpha,term_xi,term_tao,term_y,term_r,term_omega
...
!AA Added by Aditya Aiyer for Population Balance Equations
  ...
  real(kind=rprec),dimension(ld,ny,$lbz:nz)        ::temp_conc,dissip_scaled,al,bl,factor_brk
  real(kind=rprec)                                 :: a1,b1,d_0,ab_sum
  integer                                          :: counter_pbe,m,ip
  real(kind=rprec),dimension(npcon)                :: diameter,dcube,dsquare
...
!DY End here
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!AA Add diameter bins


if(PBE_FLAG) then
  ones=1._rprec
  diameter (1)=dmin
  diameter (npcon-1) = dmax
...
dcube=diameter**3
Rhs_pbe_in =0._rprec
call calculate_probability(diameter,beta)
call sgs_stag()
endif
!AA Ends Here
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!AA define variables needed for breakup calculations

if(PBE_FLAG) then
  Oh = (rho_c*nu_d**2/sigma/diameter)**(0.5_rprec)
  Ohl = log10(Oh)
  if(coord.eq.0)then
    open(unit=61,file="Oh.dat")
    do i=1,npcon
      write(61,*) Oh(i)
    enddo
  close(61)
  endif
!  a = 2.218_rprec*exp(-14.54_rprec*Oh)+2.875_rprec*exp(-0.07889_rprec*Oh)
!  b = 0.45_rprec
! ! c = -0.2339_rprec*Oh**(-2.423_rprec)/(1-0.5538_rprec*Oh**(0.5965_rprec)) -1.387_rprec
!  c = -10**(-(-0.3733_rprec*Oh**(-1.147_rprec)/(1+0.2734_rprec*Oh**(-0.8984_rprec))))
!  d = 33.08_rprec*Oh**(0.04237_rprec) -37.11
!  e = 10**(0.3637_rprec*exp(-33.29_rprec*log10(Oh+1)) + 0.5727_rprec*exp(-0.4443*log10(Oh+1)))
  a = 2.158_rprec*exp(-14.52_rprec*Oh)+2.843_rprec*exp(-0.07109_rprec*Oh)
  b = 0.45_rprec
  c = -10**((1.412_rprec*Oh**(-0.24_rprec)/(1+3.79_rprec*Oh**(-0.8936_rprec))))
  d = 21.96_rprec*Oh**(0.06538_rprec) -25.98
  e = 10**(0.3812_rprec*exp(-31.74_rprec*log10(Oh+1)) + 0.5257_rprec*exp(-0.476*log10(Oh+1)))


  if (coord .eq. 0) then
    open(unit=63,file="fit_param.dat")
    write(63,*) "variables = a,b,c,d,e"
    do ipcon=1,npcon
      write(63,'(5(e14.7,1X))') a(ipcon),b(ipcon),c(ipcon),d(ipcon),e(ipcon)
    enddo
    close(63)
  endif

  do ip=1,npcon
    dissip_scaled = dissip*u_star**3._rprec ! Scale dissipation with u_star
    tau_c(:,:,:,ip)=abs(dissip)**(0.33333333_rprec)*diameter(ip)**(-0.6666667_rprec)
    Re(:,:,:,ip)=abs(dissip_scaled)**(1._rprec/3._rprec)*diameter(ip)**(4._rprec/3._rprec)/nu_c
    Re1 = log10(Re)
    
!    fit(:,:,:,ipcon)=a(ipcon)*exp(-((log10(Re(:,:,:,ipcon))-c(ipcon))/b(ipcon))**2)* &
 !                 (1+erf(alpha(ipcon)*(log10(Re(:,:,:,ipcon))-c(ipcon))/b(ipcon)))
    factor_brk = 0.5_rprec*(sign(ones,Re1(:,:,:,ip))+1._rprec)
    al = a(ip)*factor_brk*Re1(:,:,:,ip)**(factor_brk*b(ip))
    bl =factor_brk* c(ip)*Re1(:,:,:,ip)**(factor_brk*d(ip))-e(ip)
   fitl(:,:,:,ip) = al+bl
   fit(:,:,:,ip) = 10**(fitl(:,:,:,ip))
!    if (coord.eq.0) then
     ! open(unit=64,file='fit.dat')
!      do jz=1,nz
!      
!      write(*,*)fitl(xps,yps,jz,ipcon),al(xps,yps,jz),bl(xps,yps,jz),0.5_rprec*(sign(ones(xps,yps,jz),Re1(xps,yps,jz,ipcon))+1)*Re1(xps,yps,jz,ipcon),&
!                                                    ipcon,factor_brk(xps,yps,jz)
!      enddo
      
!      close(64)
!    endif

 !   breakage_freq(:,:,:,ipcon)=k_b*pi/4*0.812*tau_c(:,:,:,ipcon)*Re(:,:,:,ipcon)**2*fit(:,:,:,ipcon)
    breakage_freq(:,:,:,ip) = k_b*(2.1_rprec)**(0.5_rprec)*pi/4._rprec*0.812_rprec*tau_c(:,:,:,ip)*fit(:,:,:,ip)
enddo

 ! a = 1.298_rprec*10**(1.992_rprec*Ohl)/(1 + 53.72_rprec*10**(1.168*Ohl))
 ! b = -0.55_rprec*Oh**(0.4763_rprec) + 1.037_rprec
  !c = 35.72_rprec*Oh**(-0.01127_rprec)-35.96_rprec
  !alpha = -1.847_rprec*Oh**(0.4735_rprec) +4.717_rprec
endif
...
!AA Begin modification of removing do loops
!  DO j=1,ny
 !   DO i=1,nx
...
!          term1=((2._rprec*z_os(i,j))/dz)**(Sc_boun*settling_vel/(vonk*ustar_local(i,j)))
  !        term1=((z_os(i,j)-d0(i,j))/(dz/2._rprec-d0(i,j)))**(Sc_boun*settling_vel(ipcon)/(vonk*ustar_local(i,j)))
   ...
   !       P_surf_flux(i,j)=((S_Surf(i,j)*term1-scalar(i,j,1,ipcon))*settling_vel(ipcon))/(1._rprec-term1)
          P_surf_flux=((S_Surf*term1-scalar(1:nx,:,1,ipcon))*settling_vel(ipcon))/(1._rprec-term1)
          ...
          ! CHange if statement to where 
           where (L .lt. 0.0_rprec)  ! For unstable atmosphere
            ...
            !END IF
            else where (L .gt. 0._rprec)
           ! IF (L(i,j) .gt. 0._rprec) THEN ! For stable atmosphere
          !  IF (L .gt. 0._rprec) THEN ! For stable atmosphere
          ! Replace IF with else where
          ...
          END IF
        P_surf_flux_dep=((0._rprec-scalar(1:nx,:,1,ipcon))*settling_vel(ipcon))/(1._rprec-term1)
        ELSE
         ...
         !P_surf_flux(i,j)=((S_Surf(i,j)-scalar(i,j,1,ipcon))*vonk*ustar_local(i,j))/(Sc_boun*term1)
          P_surf_flux=((S_Surf-scalar(1:nx,:,1,ipcon))*vonk*ustar_local)/(Sc_boun*term1)
        ! Calculate real deposition surface flux for neutral atmospheric stability
        ! This is the surface flux if there is Cr=0 everywhere in the domain and
        ! is used to characterize the deposition
          P_surf_flux_dep=((0._rprec-scalar(i,j,1,ipcon))*vonk*ustar_local(i,j))/(Sc_boun*term1)
        END IF
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      if (BUBBLE_BURST_FLAG) then
         if(ipcon.ge.ip_bubble_min.and.ipcon.le.ip_bubble_max) then
            P_surf_flux=scalar(1:nx,:,1,ipcon)*(w(1:nx,:,1)-settling_vel(ipcon))
            where(P_surf_flux > 0._rprec) P_surf_flux=0._rprec
            P_surf_flux_dep=P_surf_flux
         endif
      endif
!+++++++++++++++
...
!        dsdz(i,j,1)=(-Sc_boun*P_surf_flux(i,j)-settling_vel(ipcon)*scalar(i,j,1,ipcon))/(ustar_local(i,j)*vonk*(dz/2._rprec-d0(i,j)))
        dsdz(1:nx,:,1)=(-Sc_boun*P_surf_flux-settling_vel(ipcon)*scalar(1:nx,:,1,ipcon))/(ustar_local*vonk*(dz/2._rprec-d0))
      ELSE
     !   dsdz(i,j,1)=-(Sc_boun*P_surf_flux(i,j))/(ustar_local(i,j)*vonk*(dz/2._rprec-d0(i,j)))
        dsdz(1:nx,:,1)=-(Sc_boun*P_surf_flux)/(ustar_local*vonk*(dz/2._rprec-d0))
      END IF
      END IF
    ...
    ! Changed one if to where statement
      IF (pointsource) THEN
        where(P_surf_flux .gt. 0) P_surf_flux=0._rprec
      END IF
      
      ! For first time step, RHS(n-1)=RHS(n) for time integration
      IF ((jt_total .eq. PCon_init) .or. (pointsource .and. (jt_total .eq. ini_src))) THEN
        surf_flux_m=P_surf_flux
        surf_flux_src_m=P_surf_flux_dep
      END IF      
    ...
    ! deposition(i,j)=deposition(i,j)+(-dt*(1.5_rprec*P_surf_flux(i,j)-0.5_rprec*surf_flux_m(i,j)))
      deposition=deposition+(-dt*(1.5_rprec*P_surf_flux-0.5_rprec*surf_flux_m))

      ! Calculate pollen source
  !    Real_dep(i,j)=Real_dep(i,j)+(-dt*(1.5_rprec*P_surf_flux_dep(i,j)-0.5_rprec*surf_flux_src_m(i,j)))
      Real_dep=Real_dep+(-dt*(1.5_rprec*P_surf_flux_dep-0.5_rprec*surf_flux_src_m))
!    END DO
!  END DO
 
!AA End modification of removing do loops 
        ...
        DO k=1,nz-1
           DO j=1,ny
              ...
              DO i=3,nx-1
                 ...
                 IF ((u_int(i,j,k)+ust(k)+ucross) >= 0._rprec) THEN  
              ...
              ! Second node (i=2)
              ! Choose the appropriate upwind direction
              IF ((u_int(2,j,k)+ust(k)+ucross) >= 0._rprec) THEN  
              ...
              ! Choose the appropriate upwind direction
              IF ((u_int(nx,j,k)+ust(k)+ucross) >= 0._rprec) THEN  
       ...
       if(OCEAN_FLAG .and. STOKES_FLAG) then
          ...
          DO k=1,nz-1
             DO j=1,ny
                ...
                ! First node (i=1)
                ! Choose the appropriate upwind direction
                IF ((u_int(1,j,k)+ust(k)+ucross) >= 0._rprec) THEN  
                ...
                ! Last node (i=nx+1)
                ! Choose the appropriate upwind direction
                IF ((u_int(nx+1,j,k)+ust(k)+ucross) >= 0._rprec) THEN  
...
!DY  	RHS(i,j,k)=-(1._rprec/dx)*(u_int(i+1,j,k)*scalar_x(i+1,j,k)-u_int(i,j,k)*scalar_x(i,j,k))
         if(OCEAN_FLAG .and. STOKES_FLAG) then
            RHS(i,j,k,ipcon)=-(1._rprec/dx)*(u_int(i+1,j,k)*scalar_x(i+1,j,k)-u_int(i,j,k)*scalar_x(i,j,k)&
                 +(ust(k)+ucross)*(scalar_x(i+1,j,k)-scalar_x(i,j,k)))
         else
            RHS(i,j,k,ipcon)=-(1._rprec/dx)*(u_int(i+1,j,k)*scalar_x(i+1,j,k)-u_int(i,j,k)*scalar_x(i,j,k))
         endif
!+++++++++++++++
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  if(OCEAN_FLAG .and. STOKES_FLAG) then
     flux_out=(SUM((u_int(nx+1,1:ny,1:nz-1)+ust(k-1)+ucross)*scalar_x(nx+1,1:ny,1:nz-1))*dt*dy*dz)
  else
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!AA Add the breakage frequency calculations
  if(PBE_FLAG .AND. ipcon .NE. npcon ) then

!    dissip_scaled = dissip*u_star**3._rprec ! Scale dissipation with u_star
!    tau_c(:,:,:,ipcon)=abs(dissip)**(0.33333333_rprec)*diameter(ipcon)**(-0.6666667_rprec)
!    Re(:,:,:,ipcon)=abs(dissip_scaled)**(1._rprec/3._rprec)*diameter(ipcon)**(4._rprec/3._rprec)/nu_c
!    Re1 = log10(Re)
!    
!!    fit(:,:,:,ipcon)=a(ipcon)*exp(-((log10(Re(:,:,:,ipcon))-c(ipcon))/b(ipcon))**2)* &
! !                 (1+erf(alpha(ipcon)*(log10(Re(:,:,:,ipcon))-c(ipcon))/b(ipcon)))
!    factor_brk = 0.5_rprec*(sign(ones,Re1(:,:,:,ipcon))+1._rprec)
!    al = a(ipcon)*factor_brk*Re1(:,:,:,ipcon)**(factor_brk*b(ipcon))
!    bl =factor_brk* c(ipcon)*Re1(:,:,:,ipcon)**(factor_brk*d(ipcon))-e(ipcon)
!   fitl(:,:,:,ipcon) = al+bl
!   fit(:,:,:,ipcon) = 10**(fitl(:,:,:,ipcon))
!!    if (coord.eq.0) then
!     ! open(unit=64,file='fit.dat')
!!      do jz=1,nz
!!      
...
!
! !   breakage_freq(:,:,:,ipcon)=k_b*pi/4*0.812*tau_c(:,:,:,ipcon)*Re(:,:,:,ipcon)**2*fit(:,:,:,ipcon)
!    breakage_freq(:,:,:,ipcon) = k_b*(2.1_rprec)**(0.5_rprec)*pi/4._rprec*0.812_rprec*tau_c(:,:,:,ipcon)*fit(:,:,:,ipcon)
 !  write(str1,'(A,I6.6,A)') 'param_',jt_total,'.dat'
 !   open(104,FILE=TRIM(str1)) 
 !   do i=1,nx
 !     do j=yps,yps
!        do k=1,nz-1
!            write(104,*) dissip(i,j,k),Re(i,j,k,10),breakage_freq(i,j,k,10)
!          enddo
!        enddo
!      enddo
  !  enddo
 !   close(104) 
!AA changed loop till npcon-1
!    open (111,file="test_negative.dat",position='append')
    do jpcon=ipcon+1,npcon-1
      counter_pbe=2
      temp_conc=breakage_freq(:,:,:,jpcon)*beta(ipcon,jpcon)*scalar(:,:,:,jpcon)
 !     check_neg(:,:,:,jpcon) = -temp_conc+scalar(:,:,:,jpcon)
   !  do jx=1,nx
   !    do jy=1,ny
   !      do jz=1,nz-1 
   !        if (temp_conc(jx,jy,jz) .gt. scalar(jx,jy,jz,jpcon)) then
   !            write (111,*) "jt jx,jy,jz,temp_conc Scalar",jt,jx,jy,jz,temp_conc,scalar(jx,jy,jz,jpcon)
   !        endif
   !      enddo
   !    enddo
   !  enddo
      Rhs_pbe_in(:,:,:,ipcon) = Rhs_pbe_in(:,:,:,ipcon) + temp_conc
      Rhs_pbe_in(:,:,:,jpcon) = Rhs_pbe_in(:,:,:,jpcon) - temp_conc
      d_0 = (dcube(jpcon) - dcube(ipcon))**(1._rprec/3._rprec)
      if(d_0 .gt. diameter(1)) then
countloop:do while(counter_pbe<npcon)
          if(d_0>diameter(counter_pbe)) then
            counter_pbe=counter_pbe+1
          else
            a1=abs(diameter(counter_pbe)-d_0)
            b1=abs(diameter(counter_pbe-1)-d_0)
            ab_sum=a1+b1
            Rhs_pbe_in(:,:,:,counter_pbe-1)=Rhs_pbe_in(:,:,:,counter_pbe-1)+temp_conc*(a1/ab_sum)*(d_0**3/dcube(counter_pbe-1))
            Rhs_pbe_in(:,:,:,counter_pbe)=Rhs_pbe_in(:,:,:,counter_pbe)+temp_conc*(b1/ab_sum)*(d_0**3/dcube(counter_pbe))
            exit countloop
          endif
        enddo countloop
      else
        Rhs_pbe_in(:,:,:,1)=Rhs_pbe_in(:,:,:,1)+temp_conc*(d_0**3/diameter(1)**3)
      endif
    enddo
!  endif  
!AA End Here
...
!    do jy=1,ny
!      do jx=1,nx
!        RHS(jx,jy,jz,ipcon) = RHS(jx,jy,jz,ipcon) + Rhs_pbe_in(jx,jy,jz,ipcon)
!      enddo
!    enddo
!  enddo
close(111)
...
!open(unit=109,file="rhs.dat")
!    do i=yps,yps
     ! do j=xps,xps
      !  do k=1,nz-1
      !    do m=1,npcon
     !       write(109,*) Rhs_pbe(i,j,k,m)
    !      enddo
   !     enddo
  !    enddo
 !   enddo
...
!subroutine calculate_dissipation_energy
!
!  real(kind=rprec), dimension(:,:,:), allocatable :: e_d
 ! e_d = 2*
!
!end subroutine calculate_dissipation_energy

!subroutine calculate_breakup(beta,breakage_freq,RHS_pbe)

!  real(kind=rprec),dimension(npcon,npcon),intent(in) :: beta
!  real(kind=rprec),dimension(ld,ny,$lbz:nz_tot,npcon),intent(in) :: breakage_freq
!  real(kind=rprec),dimension(ld,ny,$lbz:nz_tot,npcon),intent(out) :: RHS_pbe
  

 

!end subroutine calculate_breakup



subroutine calculate_probability(diameter,beta)

!ef_max is the maximum formation energy associated with the generated surface
!ef_min is the minimum formation energy associated with the generated surface
!d_min is a minimum dimension in the system which is taken to be 10 micro meter.
!beta is the probability that drop j breaks up into drop of size i
  implicit none
    
  real(kind=rprec),dimension(npcon) :: diameter,dcube,dsquare
  real(kind=rprec),dimension(npcon,npcon):: beta
  real(kind=rprec),dimension(npcon,npcon) :: ef = 0._rprec
  real(kind=rprec),dimension(npcon) :: ef_max,ef_min
  real(kind=rprec) :: num,temp
  real(kind=rprec),dimension(:),allocatable :: den
  integer :: i,j,k
  beta =0._rprec
  open(unit=103,file="diameter.dat")
  do i=1,npcon
    write(103,*) diameter(i)
  enddo
  dcube = diameter**3._rprec
  dsquare=diameter**2._rprec
    do i=1,npcon
    ef_max(i) = pi*sigma*0.25992104_rprec*dsquare(i)
    ef_min(i) = pi*sigma*((dcube(i) - d_min**3)**(2._rprec/3._rprec) + d_min**2 -dsquare(i))
    do j=i+1,npcon
      ef(i,j) = pi*sigma*((dcube(j) - dcube(i))**(2._rprec/3._rprec) + &
                          dsquare(i)-dsquare(j))
    enddo
  enddo
  
!  open(unit=102,file="ef.dat")
!  do i=1,npcon
!    do j=1,npcon
!      write(102,*) ef(i,j)
!    enddo
...
!  close(102)
  do i=1,npcon-1
    do j=i+1,npcon-1
      num = ef_min(j) + ef_max(j) - ef(i,j)
      ALLOCATE (den(j-1))
      do k=1,j-1
        den(k) = ef_min(j) + ef_max(j) - ef(k,j)
      enddo
...
!  open(unit=101,file="breakage_probability.dat",ACTION="write",STATUS="replace")
!  do i=1,npcon
!    write(101,'(*(F16.9))') (beta(i,j),j=1,npcon)
!  enddo
!  close(101)
...
!subroutine breakage_frequency

!  real(kind=rprec),dimension(npcon) :: Oh,Ohl,a,b,c,alpha
!  real (rprec), dimension (ld,ny,$lbz:nz_tot,npcon) :: Re,fit,breakage_freq,tau_c 

!  Oh = (rh0_c*nu_c**2/sigma/diameter)**(0.5)
!  Ohl = log10(Oh)
!  a = 1.298_rprec*10**(1.992_rprec*Ohl)/(1 + 3.959_rprec*10**(1.005*Ohl))
!  b = -0.6327_rprec*Oh**(0.4763_rprec) + 1.057_rprec
!  c = 35.72_rprec*Oh**(-0.01127_rprec)-35.96_rprec
!  alpha = -1.847_rprec*Oh**(0.4735_rprec) +4.717_rprec

!  do ipcon=1,npcon
!    tau_c(:,:,:,ipcon) = dissip**(0.33333333_rprec)*diameter(ipcon)**(-0.6666667_rprec)
!    Re(:,:,:,ipcon) = dissip**(1/3)*diameter(ipcon)**(4/3)/nu_c
!    fit(:,:,:,ipcon) = a(ipcon)*exp(-((log10(Re(:,:,:,ipcon))-c(ipcon))/b(ipcon))**2)*(1+erf(alpha(ipcon)*(log10(Re(:,:,:,ipcon))-c(ipcon))/b(ipcon)))
!    breakage_freq(:,:,:,ipcon) = k_b*pi/4*0.812*tau_c(:,:,:,ipcon)*Re(:,:,:,ipcon)**2*fit(:,:,:,ipcon)
!  enddo
!  
!  open(unit=100,file="frequency_parameters.dat")
!  write(100,*) 'variables=x,y,z,Re1,Re2,Re3,breakage_freq1,breakage_freq2,breakage_freq3'
!  write(100,*) 'zone t="',jt_total,'" i=',nx,' j=',yps,' k=',nz,' f=point'
!  do jz=1,nz
!    do jy=yps,yps
!      do jx=1,nx
!        write(100,*) (jx-1)*L_x*z_i/nx,(jy-1)*L_y*z_i/ny,(jz-1)*z_i/nz_tot,Re(jx,jy,jz,1) &
!                     Re(jx,jy,jz,2),Re(jx,jy,jz,3),breakage_freq(jx,jy,jz,1) &
!                     breakage_freq(jx,jy,jz,2),breakage_freq(jx,jy,jz,3)
!      enddo
!    enddo
!  enddo
!  close(100)
!end subroutine breakage_frequency
