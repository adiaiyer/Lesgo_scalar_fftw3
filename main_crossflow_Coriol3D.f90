...
use param
use sim_param
use io,only:openfiles,output_loop,output_final,inflow_write,calculate_avgs,avg_stats,create_folder_for_output
...
use scalars_module2,only:patch_or_remote,timestep_conditions,ic_scal_v2,ubar,vbar,wbar,thetabar
!END
$if ($LVLSET)
  use level_set
$endif
...
integer,parameter :: n_sup = 2
character (64) :: fnamev

real(kind=rprec) rmsdivvel,kea,kestor,testu,start,finish,time_cpu   
...
!AA
real(kind=rprec) :: ordinate,cum_cputime=0._rprec,before_output,after_output,&
                    t_total_output
!ADD VARIABLE FOR BODY FORCE TERM
!REAL(KIND=RPREC) :: C1= 200
REAL(KIND=RPREC),dimension(nsteps) :: RAN_NUM
...
!AA RANDOM NUMBER INIT

call random_number(ran_num)
ran_num = 2*ran_num-1;

!AA END RANDOM NUMBER INIT
...
if(FRINGE_FLAG) then
   ...
   open(unit=200)
   do jx=1,nx
      write(200,*) jx,fringe(jx),1._rprec-fringe(jx)
   enddo
   close(200)
endif
!end
...
if (coord .eq. 0) call create_folder_for_output



do jt=1,nsteps   
  counter_out = counter_out+1
  call cpu_time(start) ! Start CPU time clock
...
!DY Added by Di Yang for ocean simulation
  if (OCEAN_FLAG) then
     ...
     if(STOKES_FLAG) then
        ...
        enddo
        if (coord .eq. 0)write(*,*) "max ust is", maxval(ust),U_stokes
        if(Coriolis3D_FLAG) then
           do jz=1,nz-1
              do jy=1,ny
                 do jx=1,nx
                    RHSx(jx,jy,jz) = RHSx(jx,jy,jz) +  2._rprec*omega_coriol &
                         *( v(jx,jy,jz)*sin(latitude) - (-w(jx,jy,jz))*cos(latitude) )
                    RHSy(jx,jy,jz) = RHSy(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) &
                         * (u(jx,jy,jz)+ust(jz))
                    RHSz(jx,jy,jz) = RHSz(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) &
                         * (u(jx,jy,jz)+ust(jz))
!!$                    RHSy(jx,jy,jz) = RHSy(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) &
!!$                         * (u(jx,jy,jz)+ust(jz)+ucross) 
!!$                    RHSz(jx,jy,jz) = RHSz(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) &
!!$                         * (u(jx,jy,jz)+ust(jz)+ucross)
                    !DY: The crossflow velocity ucross is superimposed for scalar transport
                    !DY: ucross represent effect from large scale circulation
                    !DY: Do not count in local Coriolis balance
                    !DY: The sign of w has been reversed for ocean flow because z is reversed
                    !DY: The sign of Coriolis force in z direction has also been reversed 
                    dudt(jx,jy,jz) = dudt(jx,jy,jz) +  2._rprec*omega_coriol &
                         *( v(jx,jy,jz)*sin(latitude) - (-w(jx,jy,jz))*cos(latitude) )
                    dvdt(jx,jy,jz) = dvdt(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) &
                         * (u(jx,jy,jz)+ust(jz))
                    dwdt(jx,jy,jz) = dwdt(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) &
                         * (u(jx,jy,jz)+ust(jz))
!!$                    dvdt(jx,jy,jz) = dvdt(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) &
!!$                         * (u(jx,jy,jz)+ust(jz)+ucross)
!!$                    dwdt(jx,jy,jz) = dwdt(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) &
...
!!$                    RHSy(jx,jy,jz) = RHSy(jx,jy,jz) -coriol * (u(jx,jy,jz)+ust(jz)+ucross)
                    dudt(jx,jy,jz)=dudt(jx,jy,jz)+coriol*v(jx,jy,jz)
                    dvdt(jx,jy,jz)=dvdt(jx,jy,jz)-coriol*(u(jx,jy,jz)+ust(jz))
!!$                    dvdt(jx,jy,jz)=dvdt(jx,jy,jz)-coriol*(u(jx,jy,jz)+ust(jz)+ucross)
                    !DY: The crossflow velocity ucross is superimposed for scalar transport
                    !DY: ucross represent effect from large scale circulation
                    !DY: Do not count in local Coriolis balance
                 enddo
           ...
           do jz=1,nz-1
              do jy=1,ny
                 do jx=1,nx
                    RHSx(jx,jy,jz) = RHSx(jx,jy,jz) +  2._rprec*omega_coriol* &
                         ( v(jx,jy,jz)*sin(latitude) - (-w(jx,jy,jz))*cos(latitude) )
                    RHSy(jx,jy,jz) = RHSy(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) * u(jx,jy,jz)
                    RHSz(jx,jy,jz) = RHSz(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) * u(jx,jy,jz)
!!$                    RHSy(jx,jy,jz) = RHSy(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) &
!!$                         * (u(jx,jy,jz)+ucross)
!!$                    RHSz(jx,jy,jz) = RHSz(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) &
!!$                         * (u(jx,jy,jz)+ucross)
                    ...
                    dudt(jx,jy,jz) = dudt(jx,jy,jz) +  2._rprec*omega_coriol* &
                         ( v(jx,jy,jz)*sin(latitude) - (-w(jx,jy,jz))*cos(latitude) )
                    dvdt(jx,jy,jz) = dvdt(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) * u(jx,jy,jz)
                    dwdt(jx,jy,jz) = dwdt(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) * u(jx,jy,jz)
!!$                    dvdt(jx,jy,jz) = dvdt(jx,jy,jz) - 2._rprec*omega_coriol*sin(latitude) &
!!$                         * (u(jx,jy,jz)+ucross)
!!$                    dwdt(jx,jy,jz) = dwdt(jx,jy,jz) - 2._rprec*omega_coriol*cos(latitude) &
!!$                         * (u(jx,jy,jz)+ucross)
...
!!$           RHSy(:, :, 1:nz-1) = RHSy(:, :, 1:nz-1) -coriol * (u(:, :, 1:nz-1)+ucross)
           dudt(:,:,1:nz-1)=dudt(:,:,1:nz-1)+coriol*v(:,:,1:nz-1)
           dvdt(:,:,1:nz-1)=dvdt(:,:,1:nz-1)-coriol*u(:,:,1:nz-1)
!!$           dvdt(:,:,1:nz-1)=dvdt(:,:,1:nz-1)-coriol*(u(:,:,1:nz-1)+ucross)
           !DY: The crossflow velocity ucross is superimposed for scalar transport
           !DY: ucross represent effect from large scale circulation
           !DY: Do not count in local Coriolis balance
        endif
...
!       print *,'ordinate,jz,beforeRhsz(xps,yps,jz),coord',ordinate,jz,Rhsz(xps,yps,jz),coord
    endif
  enddo
endif
...
!  enddo
  ...
  if (USE_MPI .and. coord == ncpu_source) then !LV2
    if(jt_total .le. jet_rand_final) then !LV3
      do jy=yps-2,yps+2
        do jx=xps-2,xps+2
          do jz=zps_local-1,zps_local+1
           RHSz(jx,jy,jz) = rhsZ(JX,JY,jz) - C1*(z_i/u_star**2)*exp(-(((jx-xps)*dx)**2+((jy-yps)*dy)**2+((jz-zps_local)*dz)**2)**(n_sup/2)/(2._rprec*b0_plume**n_sup))&
                                            /(((2._rprec*pi)**(3/2))*(b0_plume/dx)**3)+ ran_num(jt)/u_star**2 
 !        RHSz(jx,jy,zps_local-1:zps_local+1) = rhsZ(JX,JY,ZPS_LOCAL-1:zps_local+1) - C1/u_star**2
...
!          RHSx(jx,jy,zps_local) = RHSx(jx,jy,zps_local) 
!          RHSy(jx,jy,zps_local) = RHSy(jx,jy,zps_local) 
          enddo
        enddo
      enddo
      ...
      do jy=yps-2,yps+2
        do jx=xps-2,xps+2 
          do jz=zps_local-1,zps_local+1
             RHSz(jx,jy,jz) = rhsZ(JX,JY,jz)-C1/u_star**2*exp(-(((jx-xps)*dx)**2+((jy-yps)*dy)**2+((jz-zps_local)*dz)**2)**(n_sup/2)/(2._rprec*b0_plume**n_sup))&
                                            /(((2._rprec*pi)**(3/2))*(b0_plume/dx)**3)
          enddo
        enddo
      enddo
    endif !LV3
...
if (Jet) then
     do jz=1,nz-1
       ordinate = coord*(nz-1)+jz
       if ((ordinate .GT. zps-2) .AND. (ordinate .LT. zps+2)) then
 !         print *,'ordinate,jz,afterRhsz(xps,yps,jz),coord',ordinate,jz,Rhsz(xps,yps,jz),coord
       endif
     enddo
  endif
!AA END her
...
!    ordinate = coord*(nz-1)+jz
!    if ((ordinate .GT. zps-2) .AND. (ordinate .LT. zps+2)) then
!      RHSZ(XPS-1:XPS+1,YPS-1:YPS+1,jz)=RHSZ(XPS-1:XPS+1,YPS-1:YPS+1,jz)-C1/u_star
!    endif
!  enddo
!endif
...
!!$                 + ubar(jz)*fringe(jx)
!!$            v(jx,jy,jz) = v(ix_fringe,jy,jz)*(1._rprec-fringe(jx)) &
!!$                 + vbar(jz)*fringe(jx)
!!$            w(jx,jy,jz) = w(ix_fringe,jy,jz)*(1._rprec-fringe(jx)) &
!!$                 + wbar(jz)*fringe(jx)
              u(jx,jy,jz) = u(ix_fringe,jy,jz)*(1._rprec-fringe(jx)) &
                   + 0._rprec*fringe(jx)
              v(jx,jy,jz) = v(ix_fringe,jy,jz)*(1._rprec-fringe(jx)) &
              ...
              w(jx,jy,jz) = w(ix_fringe,jy,jz)*(1._rprec-fringe(jx)) &
                   + 0._rprec*fringe(jx)
              theta(jx,jy,jz) = theta(ix_fringe,jy,jz)*(1._rprec-fringe(jx)) &
                   + thetabar(jz)*fringe(jx)
           enddo
        enddo
     enddo
  endif
...
!  if (mod (counter_out,wbase)==0) call calculate_avgs
...
!!AA For checking output time

   call cpu_time(before_output)

    call output_loop ()
    !  call output_slice_loop()
    call cpu_time(after_output)
   t_total_output = after_output-before_output
  
  
    If(coord .eq. 0) then
    write(*,*) 'TIME OUTPUT=', t_total_output
    end if
!! AA END CHECK
...
!DY End here
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  call cpu_time(finish) ! Stop clock
  time_cpu = finish-start !This gives the cpu time for the timestep
  open(unit=20,file ="cpu_timedata.dat")
  write(20,*) jt,time_cpu
20  format(1X,I5,F10.5)
  
  cum_cputime = cum_cputime+time_cpu  
end do  !--end time loop
close(20)
open(unit=21,file = "final_cpu_time.dat")
write(21,*) cum_cputime
close(21)
