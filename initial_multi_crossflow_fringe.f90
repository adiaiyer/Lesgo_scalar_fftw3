...
use scalars_module2,only:ic_scal_v2,ubar,vbar,wbar,thetabar!,ic_scal_gabls,ic_scal_GABLS_diurnal ! added by VK
...
if(initu)then
  ...
  write(fname,'(A,A,I4.4,A)')path//'restart/vel_pcon_0040000','_',2000+coord,'.out'
   else if(S_FLAG) then
...
!   if(initsc) then
   ...
   if(S_FLAG) then
   write(fname,'(A,A,I4.4,A)') path//'restart/fringe_vel_sc_0040000','_',2000+coord,'.out'
   else
      write(fname,'(A,I4.4,A)') path//'restart/fringe_vel_',2000+coord,'.out'
   endif
   open(unit=2000+coord,file=fname,form='unformatted')
   if(S_FLAG) then
      read(2000+coord) ubar(:),vbar(:),wbar(:),thetabar(:)
   else
      read(2000+coord) ubar(:),vbar(:),wbar(:),thetabar(:)
   endif
   !DY End here
